...
jQuery.fn.checkTree = function(settings) {
    ...
    $tree.find("li")
        ...
        .each(function() {
            ...
            var nestedCheckboxes        = jQuery(this).find(':checkbox');
            var checkedNestedCheckboxes = jQuery(nestedCheckboxes).filter(':checked');
            if (checkedNestedCheckboxes.size() > 0) {
                if (checkedNestedCheckboxes.size() === nestedCheckboxes.size()) {
